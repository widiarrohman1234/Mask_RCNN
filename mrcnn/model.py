...
class MaskRCNN(object):
    ...
    def load_weights_old(self, filepath, by_name=False, exclude=None):
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        """Load weights from an HDF5 file, compatible with TF 2.x eager mode.
        Supports excluding certain layers.
        """
        import h5py
        import numpy as np
        ...
        with h5py.File(filepath, mode='r') as f:
            # Handle Keras 'model_weights' group if present
            if 'layer_names' not in f.attrs and 'model_weights' in f:
                f = f['model_weights']

            # Get layers from inner model if multi-GPU
            keras_model = self.keras_model
            ...
            if by_name:
                # Build index of layers in file
                layer_names = [n.decode('utf8') for n in f.attrs['layer_names']]
                layer_dict = {name: f[name] for name in layer_names}

                for layer in layers:
                    g = layer_dict.get(layer.name)
                    if g is None:
                        continue
                ...
                # Sequential/functional exact match
                weight_value_tuples = []
                for layer in layers:
                    g = f[layer.name]
                    weight_names = [n.decode('utf8') for n in g.attrs['weight_names']]
                    weight_values = [np.array(g[weight_name]) for weight_name in weight_names]
                    if weight_values:
                        weight_value_tuples.append((layer, weight_values))

                for layer, values in weight_value_tuples:
                    try:
    ...
    def compile(self, learning_rate, momentum):
        ...
        for name in loss_names:
            ...
            loss = (keras.ops.mean(layer.output, axis=None, keepdims=True)
                * self.config.LOSS_WEIGHTS.get(name, 1.))
            # old
        ...
        # saved
        self.keras_model.compile(optimizer=optimizer, loss=reg_losses)
        # experiment
        # self.keras_model.compile(optimizer=optimizer, loss=reg_losses)
        ...
        for name in loss_names:
            ...
            #     * self.config.LOSS_WEIGHTS.get(name, 1.))
            loss = (
                keras.ops.mean(layer.output, axis=None, keepdims=True)
                * self.config.LOSS_WEIGHTS.get(name, 1.))
            # old
            ...
            # self.keras_model._losses.append(loss)  # kalau mau ikut dihitung di total loss
            # self.keras_model._compile_metrics.update_state(loss)  # manual update metric
            self.keras_model.compile(metrics=[loss])
